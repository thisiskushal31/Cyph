...
import { ApiService, AllowedUserDto } from '../../core/services/api.service';
...
@Component({
  ...
  template: `
    <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <p class="mt-1 text-slate-600">Users can sign in via SSO or be added here and sign in with Google. Toggle admin to allow access to this panel.</p>

      <div class="mt-8">
        <h2 class="text-lg font-semibold text-slate-800">Add user (email)</h2>
        <form (ngSubmit)="onAdd()" class="mt-2 flex flex-wrap items-end gap-2">
          <input
            type="email"
            [(ngModel)]="newEmail"
            name="newEmail"
      ...
      <div class="mt-8">
        ...
        <div class="mt-2 overflow-hidden rounded-lg border border-slate-200 bg-white shadow">
...
export class AdminComponent implements OnInit {
  ...
  onAdd(): void {
    ...
    this.addError = null;
    this.adding = true;
    this.api.addUser(email).subscribe({
      next: () => {
        this.newEmail = '';
        this.adding = false;
        this.loadUsers();
