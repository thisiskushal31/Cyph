  ...
  template: `
    <div class="w-full max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold tracking-tight text-slate-100 sm:text-3xl">Audit log</h1>
          <p class="mt-2 text-slate-400">Pretty JSON per event. Newest first.</p>
        </div>
        <div class="flex items-center gap-2 rounded-full bg-slate-800/90 border border-slate-600/80 px-4 py-2 text-sm font-medium text-slate-300">
      ...
      <div class="mt-6 space-y-4">
        @if (loading) {
          <div class="app-card p-12 text-center">
            <div class="inline-flex h-10 w-10 animate-spin rounded-full border-2 border-slate-200 border-t-indigo-500"></div>
            <p class="mt-4 text-sm text-slate-500">Loading log…</p>
          </div>
          ...
          <div class="app-card p-12 text-center">
            ...
            <p class="mt-4 text-sm font-semibold text-slate-700">No entries yet</p>
          ...
          <div class="overflow-auto max-h-[65vh] min-h-[12rem] space-y-4 pr-1">
            @for (e of entries; track e.occurredAt + (e.messageId ?? '') + e.eventType) {
              <article class="log-entry app-card p-5 transition hover:shadow-md">
                <div class="mb-3 flex flex-wrap items-center gap-2">
                  <span class="inline-flex items-center rounded-xl bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700">{{ e.occurredAt }}</span>
                  <span class="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600">{{ e.eventType }}</span>
                </div>
      ...
      @if (page && page.totalPages > 1) {
        <div class="mt-6 app-card flex flex-wrap items-center justify-between gap-4 px-5 py-4">
          <p class="text-sm text-slate-600">
            <span class="font-semibold text-slate-800">Page {{ (page.number ?? 0) + 1 }}</span>
          ...
          <div class="flex gap-2">
            <button
              ...
              class="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 hover:border-slate-300 disabled:pointer-events-none disabled:opacity-50"
            >
              Previous
            </button>
            <button
